...
describe("getBorderStyleForDate", () => {
  it("returns blue border when the date is less than five days ago", () => {
    const today = Date.now();
    const recentDate = new Date(Date.now() - 86400000 * 3);

    const expected = "2px solid blue";
    const actual = getBorderStyleForDate(recentDate, today);

    expect(actual).to.equal(expected);
  });
  it("returns red border when the date is more than five days ago", () => {
    const today = Date.now();
